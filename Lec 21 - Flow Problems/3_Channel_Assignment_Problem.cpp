...
bool bpm(int table[M][N], int u, bool seen[], int matchR[]) {
    ...
    for (int v = 0; v < N; v++) {
        // If sender u has packets to send to receiver v and receiver v is not
        // already mapped to any other sender just check if the number of
        // packets is greater than '0' because only one packet can be sent in
        // a time frame anyways
        if (table[u][v]>0 && !seen[v]) {
            seen[v] = true; // Mark v as visited
            // If receiver 'v' is not assigned to any sender OR previously
            // assigned sender for receiver v (which is matchR[v]) has an
            // alternate receiver available. Since v is marked as visited in
            // the above line, matchR[v] in the following recursive call will
            // not get receiver 'v' again
...
// Returns maximum number of packets that can be sent parallely in 1 time slot
