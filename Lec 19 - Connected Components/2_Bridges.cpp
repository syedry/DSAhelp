...
network and are useful for designing reliable networks. For example, in a
wired computer network, an articulation point indicates the critical
...
edge causes disconnected graph.
...
DFS tree) is bridge if there does not exist any other alternative to reach u
or an ancestor of u from subtree rooted with v. As discussed in the previous
code, the value low[v] indicates earliest visited vertex reachable from subtree
rooted with v. The condition for an edge (u, v) to be a bridge is,
'low[v] > disc[u]'.
*/
...
// DFS based function to find all bridges. It uses recursive func bridgeUtil()
void Graph::bridge() {
