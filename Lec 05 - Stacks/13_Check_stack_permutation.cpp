...
There are a huge number of permutations possible using a stack for a single
input queue. Given two arrays, both having unique elements, one represents
the input queue and the other represents the output queue. Our task is to
check if the given output is possible through stack permutation.
...
The idea to do this is we will try to convert the input queue to output queue
using a stack, if we are able to do so then the queue is permutable otherwise not.
...
2. Once we find an element in input queue such the top of input queue is equal
   ...
   output queues, and compare the top of stack and top of output queue now.
   If top of both stack and output queue are equal then pop element from both
...
bool checkStackPermutation(int ip[], int op[], int n) {
    ...
    // If after processing, both input queue and stack are empty then the
    // input queue is permutable otherwise not.
